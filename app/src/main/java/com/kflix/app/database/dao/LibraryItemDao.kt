...
interface LibraryItemDao {
    ...
    @Query("SELECT * FROM library_items WHERE profileId = :profileId ORDER BY addedAtMillis DESC")
    fun getAllForProfile(profileId: String): Flow<List<LibraryItem>>

    @Query("SELECT * FROM library_items WHERE profileId = :profileId ORDER BY addedAtMillis DESC")
    fun getAllForProfileSync(profileId: String): List<LibraryItem>

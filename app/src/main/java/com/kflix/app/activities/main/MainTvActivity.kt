package com.kflix.app.activities.main
...
import com.kflix.app.databinding.ActivityMainTvBinding
import com.kflix.app.databinding.ContentHeaderMenuMainTvBinding
import com.kflix.app.fragments.player.PlayerTvFragment
import com.kflix.app.ui.UpdateAppTvDialog
import com.kflix.app.providers.Provider
import com.kflix.app.providers.Cine24hProvider
        ...
        if (savedInstanceState == null) {
            ...
            if (provider != null) {
                lifecycleScope.launch(Dispatchers.IO) {
                    val db = LibraryDatabase.getInstance(this@MainTvActivity)
                    val profiles = db.profileDao().getAllSync()
                    val currentId = UserPreferences.currentProfileId
                    val hasValidProfile = currentId != null && profiles.any { it.id == currentId }

                    val destinationId = if (profiles.isEmpty() || !hasValidProfile) {
                        R.id.profile_picker
                    } else {
                        R.id.home
                    }
        ...
        navController.addOnDestinationChangedListener { _, destination, _ ->
            binding.navMain.headerView?.apply {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    val profileName = if (profileId != null) {
                        db.profileDao().getById(profileId)?.name
                    } else null
                    withContext(Dispatchers.Main) {
                        header.tvNavigationHeaderSubtitle.text = profileName
                            ?: getString(R.string.main_menu_change_provider)
                    }
                }

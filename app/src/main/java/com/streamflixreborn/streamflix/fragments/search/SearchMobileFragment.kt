...
import com.streamflixreborn.streamflix.databinding.ItemSearchHistoryChipBinding
...
class SearchMobileFragment : Fragment() {
    ...
    private lateinit var historyChipsAdapter: HistoryChipsAdapter
    ...
    private fun initializeSearch() {
        ...
        historyChipsAdapter = HistoryChipsAdapter(
            onClick = { query ->
                binding.etSearch.setText(query)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.recentSearches.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED).collect { history ->
                historyChipsAdapter.submitList(history)
                updateSearchHistoryVisibility()
            }
        }
    }
    ...
    private fun updateSearchHistoryVisibility() {
        ...
        val hasHistory = viewModel.recentSearches.value.isNotEmpty()
        val showHistory = query.isEmpty() && hasHistory
        binding.clRecentSearchesHeader.visibility = if (showHistory) View.VISIBLE else View.GONE
        binding.rvRecentSearches.visibility = if (showHistory) View.VISIBLE else View.GONE
    }
...
internal class HistoryChipsAdapter(
    private val onClick: (String) -> Unit
) : RecyclerView.Adapter<HistoryChipsAdapter.ChipViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ChipViewHolder, position: Int) {
        ...
        holder.binding.tvChip.text = query
        holder.itemView.setOnClickListener { onClick(query) }
    }

    class ChipViewHolder(val binding: ItemSearchHistoryChipBinding) :

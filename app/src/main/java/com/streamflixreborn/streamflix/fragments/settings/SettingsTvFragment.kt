...
class SettingsTvFragment : LeanbackPreferenceFragmentCompat() {
    ...
    private fun displaySettings() {
        ...
        findPreference<SwitchPreferenceCompat>("key_backup_auto_tv")?.apply {
        ...
        findPreference<ListPreference>("key_backup_interval_tv")?.apply {
        ...
        findPreference<Preference>("key_backup_manage_tv")?.setOnPreferenceClickListener {
            showBackupManageDialogTv()
            true
        }

        findPreference<Preference>("key_backup_restore_local_tv")?.setOnPreferenceClickListener {
            showLocalBackupRestoreDialogTv()
            true
        }
    }

    private fun updateAutoBackupLastTimeTv() {
        ...
        val pref = findPreference<Preference>("key_backup_auto_last_tv")
        if (lastBackup != null) {
    ...
    private fun showBackupManageDialogTv() {
        val backupDir = java.io.File(requireContext().filesDir, "auto_backups")
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setItems(names) { _, which ->
                ...
                AlertDialog.Builder(requireContext())
                    ...
                    .setItems(optionItems) { _, opt ->
                        when (opt) {
                            0 -> restoreFromLocalBackupTv(selectedFile)
                            1 -> {
    ...
    private fun showLocalBackupRestoreDialogTv() {
        val backupDir = java.io.File(requireContext().filesDir, "auto_backups")
        val files = backupDir.listFiles()
            ?.filter { it.name.startsWith("auto_backup_") && it.name.endsWith(".json") }
            ?.sortedByDescending { it.lastModified() } ?: emptyList()
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setItems(names) { _, which ->
                val selectedFile = files[which]
                restoreFromLocalBackupTv(selectedFile)
            }
            .setNegativeButton(android.R.string.cancel, null)
            .show()
    }

    private fun restoreFromLocalBackupTv(file: java.io.File) {
        viewLifecycleOwner.lifecycleScope.launch {
            try {

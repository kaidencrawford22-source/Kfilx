...
class BackupRestoreManager(
    ...
    private suspend fun importLibraryData(lib: JSONObject) {
        val libraryDb = LibraryDatabase.getInstance(context)
    ...
    fun performAutoBackup(): String? {
        return try {
            val jsonData = exportUserData() ?: return null
            ...
            cleanupOldAutoBackups(backupDir)
            Log.d(TAG, "Auto-backup saved to ${file.absolutePath}")
            file.absolutePath
        } catch (t: Throwable) {
            Log.e(TAG, "Error during auto-backup", t)
            null
        }
    }

    private fun cleanupOldAutoBackups(backupDir: File) {

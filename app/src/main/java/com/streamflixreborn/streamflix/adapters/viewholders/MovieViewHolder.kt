...
import com.streamflixreborn.streamflix.models.best
...
class MovieViewHolder(
    ...
    private fun displayMovieMobile(binding: ContentMovieMobileBinding) {
        ...
        binding.btnMovieDownloadSeal.apply {
            setOnClickListener {
                checkProviderAndRun {
                    itemView.findViewTreeLifecycleOwner()?.lifecycleScope?.launch(Dispatchers.IO) {
                        try {
                            ...
                            val servers = provider.getServers(
                                movie.id,
                                Video.Type.Movie(
                                    id = movie.id,
                                    title = movie.title,
                            ...
                            val video = provider.getVideo(servers.best())
                            val intent = Intent(context, VideoDownloadService::class.java).apply {
                                putExtra(VideoDownloadService.EXTRA_URL, video.source)
                                putExtra(VideoDownloadService.EXTRA_TITLE, movie.title)
                                putExtra(VideoDownloadService.EXTRA_CONTENT_ID, movie.id)
                                if (video.headers is HashMap) {
                                    putExtra(VideoDownloadService.EXTRA_HEADERS, video.headers as HashMap)
                                }
                            }

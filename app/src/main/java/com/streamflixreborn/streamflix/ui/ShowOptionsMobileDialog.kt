                        ...
                        try {
                            ...
                            val episodeVideoType = Video.Type.Episode(
                                ...
                                tvShow = Video.Type.Episode.TvShow(
                                    id = epTvShow?.id ?: "",
                                    title = epTvShow?.title ?: "",
                                    poster = epTvShow?.poster,
                                    banner = epTvShow?.banner,
                                    releaseDate = epTvShow?.released?.format("yyyy-MM-dd"),
                                    imdbId = epTvShow?.imdbId
                                ),
                                season = Video.Type.Episode.Season(
                                    number = epSeason?.number ?: 0,
                                    title = epSeason?.title
                                )
                            )
                            val servers = provider.getServers(episode.id, episodeVideoType)
                            if (servers.isEmpty()) return@launch
                            val video = provider.getVideo(servers.best())
                            val epTitle = "S${epSeason?.number ?: 0}E${episode.number} - ${episode.title ?: epTvShow?.title ?: "Episode"}"
                            val intent = Intent(context, VideoDownloadService::class.java).apply {
                                putExtra(VideoDownloadService.EXTRA_URL, video.source)
                                putExtra(VideoDownloadService.EXTRA_TITLE, epTitle)
                                putExtra(VideoDownloadService.EXTRA_CONTENT_ID, episode.id)

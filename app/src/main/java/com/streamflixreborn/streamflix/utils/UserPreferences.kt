...
object UserPreferences {
    ...
    var customAccentColor: String
        get() = Key.CUSTOM_ACCENT_COLOR.getString() ?: "default"
        set(value) = Key.CUSTOM_ACCENT_COLOR.setString(value)

    var recentSearchHistory: List<String>
        get() {
            val json = Key.RECENT_SEARCH_HISTORY.getString() ?: return emptyList()
            val arr = runCatching { JSONArray(json) }.getOrNull() ?: return emptyList()
            return (0 until arr.length()).map { arr.optString(it, "") }.filter { it.isNotEmpty() }
        }
        set(value) {
            val arr = JSONArray(value.take(10))
            Key.RECENT_SEARCH_HISTORY.setString(arr.toString())
        }
    ...
    var backupInterval: String
        get() = Key.BACKUP_INTERVAL.getString() ?: "daily"
        set(value) = Key.BACKUP_INTERVAL.setString(value)
}
